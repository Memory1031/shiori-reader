...
进度晚写保护使用 `beginProgressSession(NovelKey) → Result<int>` 与 `ProgressWriteStamp(generation,sequence)`：仓库生成每本书跨进程单调代次，调用方在同代次递增 sequence；saveProgress 原子检查 stamp 并写入。返回 true 表示已提交，false 表示旧代次 / 重复 / 倒序写被忽略，不能当作已保存。目录依据暂缺或冲突返回可手动重试的 `Failure(cache, catalogBasisUnavailable)`，保留待保存位置；它不使当前 generation 失效。代次与正文 revision、时间戳无关，不塞进 ReadingProgress 实体；存储层负责持久代次与串行事务，协议测试和设备重启证据分开记录。
...
`AppUpdateRepository` 由应用根拥有，页面借用同一个 `UpdateController`。更新检查与小说 Source 的网络服务独立；领域候选只暴露发布身份、大小、纯文本说明和发布页，不暴露缓存文件路径。候选须通过内置公钥验签及平台资产校验；下载完成核对大小与摘要，重启恢复时重新验签并核对文件。

检查与下载共用互斥操作状态，取消等待 I/O 结束后完成；根关闭时先取消并等待操作，再关闭更新网络。检查预算内找到有效升级时，返回已验证的最高构建号候选；未穷尽历史且没有有效候选时报告检查不完整。渠道及自动检查偏好独立持久化；自动检查按当前渠道上次尝试间隔 24 小时，切换渠道使检查结果和尝试时间失效，服务端限流等待仍然保留。正式包核对原生版本与包内身份后启用更新，开发构建不访问正式更新源。

Android 安装交接前重新核对签名清单与包摘要，原生层再核对包名、版本、递增构建号及 APK 证书与已安装应用的一致性。安装权限由用户授权，返回后需再次点击安装；已提交的 `PackageInstaller` 会话由系统确认或取消，页面的下载取消操作不撤销安装。原生安装记录跨进程保留，重启以实际安装版本确认结果并清理过期下载；中断留下的本应用未提交会话会被清理以允许重试。取消或可重试安装失败保留完整下载；本地包失效恢复下载入口，清单或安装身份校验失败则要求重新检查。更新页通过首页菜单或桌面工作区进入，阅读中的通知要求先走正常保存退出流程。

Windows 更新程序使用结构化任务交接，在安装目录之外运行；替换前核对进程、安装身份、签名清单及完整解压结果。拒绝绝对路径、目录穿越、符号链接 / junction、重复或大小写冲突路径以及超限解压内容。事务只管理当前与目标程序清单覆盖的文件，保留其他文件；持久记录支持未完成替换恢复。主程序先保存阅读位置和设置、释放存储与媒体资源，更新程序等待对应进程退出后替换程序。自动回滚仅适用于新应用尚未开始数据迁移的阶段；迁移后故障保留备份并按数据恢复流程处理，不能直接用旧程序读取新 schema。更新写入范围限于已校验程序目录和专用暂存 / 备份目录，不接管用户数据库、书籍或缓存。
