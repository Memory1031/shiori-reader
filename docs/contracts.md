...
Android 安装交接前重新核对签名清单与包摘要，原生层再核对包名、版本、递增构建号及 APK 证书与已安装应用的一致性。安装权限由用户授权，返回后需再次点击安装；已提交的 `PackageInstaller` 会话由系统确认或取消，页面的下载取消操作不撤销安装。原生安装记录跨进程保留，重启以实际安装版本确认结果并清理过期下载；中断留下的本应用未提交会话会被清理以允许重试。取消或可重试安装失败保留完整下载；本地包失效恢复下载入口，清单或安装身份校验失败则要求重新检查。更新页从书架进入，阅读中的通知要求先走正常保存退出流程。
