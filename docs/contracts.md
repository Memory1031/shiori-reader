...
检查与下载共用互斥操作状态，取消等待 I/O 结束后完成；根关闭时先取消并等待操作，再关闭更新网络。渠道及自动检查偏好独立持久化；自动检查按上次尝试间隔 24 小时，手动检查也遵守服务端限流等待。正式包核对原生版本与包内身份后启用更新，开发构建不访问正式更新源。

Android 安装交接前重新核对签名清单与包摘要，原生层再核对包名、版本、递增构建号及 APK 证书与已安装应用的一致性。安装权限由用户授权，返回后需再次点击安装；已提交的 `PackageInstaller` 会话由系统确认或取消，页面的下载取消操作不撤销安装。原生安装记录跨进程保留，重启以实际安装版本确认结果并清理过期下载；取消或失败保留完整下载包供重试。更新页从书架进入，阅读中的通知要求先走正常保存退出流程。
