...
移出书架不删除原件或历史；清缓存不删除用户托管文件。删除本地书籍需确认，清理该书托管数据、书架和进度，并失效旧会话晚写；外部原文件不动。
...
原生正文支持段落、标题、分隔线、换行、pre 文本、基础对齐与 em 缩进；支持 PNG / JPEG / GIF / WebP，SVG 包裹的位图可提取，纯 SVG 不在兼容范围。图片缺失显示占位，不丢失前后正文。
