...
| 11. CI | 增加 Windows build CI | `windows-latest` + 项目固定 Flutter 版本；外部 Actions 继续完整 SHA pin | Windows release build 可在 CI 稳定完成 | P1 | ◐ 已接入独立 Windows Release 构建 job，含严格依赖安装、长路径和运行目录检查；远端 runner 验收待完成 |
