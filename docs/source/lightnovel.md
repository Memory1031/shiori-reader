...
正文只读取 body_snapshot：优先非空 body_html，缺失 / 空白时可用 snapshot.body_text。**render_preview 不能回退成全文**；即使某次样本长度相同，也不能推广为所有章节完整。锁定、登录表单、缺 snapshot 或无可读块均不冒充正文。段落 / 标题 / 图片顺序保留，Ruby 降级为基字加括注，原生不支持的装饰不进入领域。
