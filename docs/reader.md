...
> iOS 状态更新（2026-09-08）：Mac / Simulator 已可用，当前证据见 [IOS-001 报告](validation/ios-001.md)。本次仅验证正式应用启动，未补验阅读器排版、手势、恢复、图片或性能。下方带日期的 `DEFERRED_NO_MAC` 等结论是当次历史记录，不代表当前环境。
...
新默认排版 18sp、1.7 行高、8dp 段距、20dp 左右边距；已有设置保留，Aa 内可恢复默认排版。源起始空白阻止额外首行缩进，UI 缩进上限 2em；文本及位置身份不变。章节 / 次级标题识别与测量渲染共享，保持两种阅读模式定位一致。顶部简化为返回、章节标题、更多；底部目录、进度、Aa。详情及重试进入更多菜单，上一章 / 下一章进入进度 Sheet。隐藏时无残留应用按钮，中部点按或 F2 唤回；系统状态栏仍保留。进度滑块按 block index / fraction 恢复，不将其描述为精确页码。修复从菜单打开详情时 PopupRoute 退场造成的旧 Reader 残留。全量离线 281 PASS；细节与视觉边界见 [应用 UI 规范](app.md#ui-规范)。

### EPUB 单页单元跨章翻页（2026-09-08）

用户 EPUB 可解析为 32 个 spine 阅读单元，封面仅一个图片块。原分页视口在相邻页不存在时直接返回，造成封面无法继续翻页。新增边界回调，左右点击 / 键盘翻页及超过 48px 的主要水平滑动可切换相邻章节；加载 / 布局不自动触发，整本首尾仍保持边界。跨章继续走 BookReaderScreen 的保存与并发切换保护，下一章从开头进入，上一章从末页进入。末页定位使用显式入口标记，不改变普通已存进度的恢复语义；滚动模式没有新增自动跨章手势。

验证：阅读器 68 项回归通过，随后新增末页入口用例并复跑分页 6 项通过；flutter analyze 无问题。iPhone 17 / iOS 26.5 模拟器运行临时探针，读取用户实际 EPUB 并使用生产 EpubParser 与 PagedReaderViewport，结果为 chapters=32、coverBlocks=1、next=true、back=true、swipe=true、turns=[1,-1,1]。这是解析器 / 视口在模拟器上的验证，未冒称完整导入、数据库保存或真机链路本轮重验。探针独立切换视口，生产会话保存接线由组件回归覆盖。书籍、探针和截图不提交；原 EPUB 未修改。

当前用户真机 scheme 为 Release，模拟器目标解析失败；使用临时 Debug scheme 和命令级平台覆盖构建通过，没有改个人团队。验证结束恢复 lib/main.dart 入口，临时 scheme 删除。iPhone 上需要重新编译安装才包含修复。

### 单 EPUB 的“分卷目录”调查（2026-09-08）

同一用户 EPUB 在 iOS 26.5 模拟器通过 ManagedLocalBooks.importBook + BookDecoder 导入、加入书架，再从 LocalBookStore.read 读取持久化内容，结果 imported=true、persisted=true、volumes=[{synthetic:true,chapters:32}]、navigation=17、nested=false。使用独立 epub-import-inspect 存储目录，未覆盖原有书架；随后显示 ProductionApp。未通过系统文件选择器操作，此验证覆盖导入解析 / 落盘 / 读回。

该文件 NCX 为 17 个平级导航项，没有分卷层级；内部一个 isSynthetic=true 的 Volume 用于统一 Catalog 合约，不应被当成真实卷，CatalogView 已隐藏该合成组标题。“分卷目录”来自 ReaderScreen 打开的 showArticleContents 面板，其 onVolumes 按钮无条件使用 volumesTitle；本地书点击后实际跳到 LocalCatalogScreen 的全书目录。结论为通用阅读面板命名误导，不是 EPUB 被拆成 32 卷。建议本地书使用“全书目录 / 本章目录”命名，保留源书既有分卷语义。本轮仅调查，没有修改文案或模型。

### 本地目录称呼适配（2026-09-08）

本地 TXT / EPUB 的阅读面板改用“本章目录 / 全书目录”，无小节时提示通过全书目录选择章节；全书目录页标题同步。中英文资源均已更新，在线书源保留原有卷内 / 分卷命名；不改 EPUB 导航结构、合成分组或进度身份。目录、书籍阅读及本地阅读 8 项组件回归通过；本轮未重新安装设备，需重新编译查看新文案。

### 本地书目录直达（2026-09-08，取代上一条交互）

按用户修正，本地 TXT / EPUB 点击阅读工具栏“目录”后直接调用完整本地导航，不再弹出本章小标题面板。“本章目录”相关中英文资源移除，保留完整目录页标题“全书目录”；在线书源仍可使用卷内标题面板。更新本地阅读测试，从真实工具栏点击目录，断言直接出现 LocalNavigationView 且没有 BottomSheet；分页 / 滚动模式与在线阅读相关共 8 项回归通过。本轮未重新安装手机。


## 阅读体验逐项优化（2026-09-08）

用户以 Apple Books 截图作为排版参考，并要求优化阅读设置底部面板。本轮交付第一批基础视觉调整：
- 新建/恢复默认排版采用左右 40、段间距 20；字号 20、行高 1.6，已有偏好不覆盖。按用户提供的 Apple Books 截图约 10% 单侧留白及每行约 16–18 个汉字估算，取代首轮 28/12；属于视觉近似，尚未宣称设备上完全对齐。
- 隐藏工具栏时保留淡色居中当前内容标题，单行省略；使用现有顶部留白，不因切换工具栏重新分页。
- 主标题与小标题加强字号层级并增加上下留白；Day 副标题继续弱化。分页测量、双模式绘制、滚动锚点偏移共享块间距。
- 设置弹窗使用圆角纸色背景、固定标题/关闭按钮、拖动提示、独立滚动内容、排版滑块分组与纸色样本；保留实时预览、保存失败重试与设置持久化。

后续项仍待处理：边距快捷档位、EPUB 场景空白与样式映射、页脚进度、插图/封面分型、中文断行评估、标题与后文同页规则。当前没有宣称完整还原 EPUB CSS 或全书页码。

验证：首轮设置/主题 14 项通过；整组阅读器回归 68 项通过、1 项 Day 副标题断言失败，已修正该层级，所在 article_contents 测试文件 5 项复验全部通过。静态分析无问题。本轮尚未安装到模拟器或真机，视觉效果待运行验收。

默认值二次调整验证：设置组件 6 项通过；同步旧默认字号断言后，领域模型 16 项复验通过。尚未进行设备截图对照。


### 第三项：EPUB 样式与特殊页面兼容

状态：TODO（2026-09-08 用户确认纳入）。兼容范围从正文样式扩展至书内使用 HTML/CSS 构建的扉页。

已确认原因：用户 EPUB 的第二个 spine 页面 `OEBPS/Text/title.xhtml` 引用 `../Styles/style.css`，每字独立 p，通过 float 左右分组、不同字号/颜色、负 margin、边框、背景及旋转组合扉页；并非漏载图片。当前 EpubParser 将段落转成 ParagraphBlock，未保留该布局与样式，导致逐字向下排列、内容溢出到后页。页面 `<title>` 为通用“标题”，也不适合作为书名页眉。

实施及验收要求：
- 保留并支持该类页面所需的容器分组、位置关系、字号、颜色、边框、背景和旋转；不得按这本书的文字或类名硬编码特例。
- 同时处理正文标题层级、段落/场景留白与基础样式映射；普通正文继续支持用户排版配置。
- 扉页完整呈现书名、装饰条及作者，分栏位置和阅读顺序接近参考图，不再逐字作为普通段落分页；页眉使用合理的书名/卷名，避免通用“标题”。
- 核查导入后的缓存升级/重新解析路径，不能仅让新导入文件生效而留下旧缓存问题。
- 以用户本地文件做离线运行对照，另用自建最小 XHTML/CSS 样本验证通用性；用户书籍内容不提交到仓库。
- 验证 Android / iOS 的显示、跨页操作和进度恢复，分别记录实际证据；同时回归普通正文排版与性能。

实现方案待评估，不能把仅改间距或拼接文字视为兼容完成；尚未完成实现、模拟器或真机验收。


### 第二项：页眉、页脚与图文分页（2026-09-08）

- 正式书籍阅读页从 cacheOnly 详情读取书名；真实卷目录补卷名，synthetic 卷不显示。无缓存时回退当前内容标题，不新增在线请求。请求随页面销毁取消。
- 隐藏工具栏时保留页眉与淡色“本章约百分比”页脚，复用 4Hz 位置通知；不推算全书页码，显示/隐藏不改变正文可用高度。
- 按当前排版高度达到正文视口 60% 的大图单独占页；前后翻页均隔离正文，小图继续流排。使用现有等比完整展示和未知尺寸重布局，不依赖文件名猜测封面，不裁切图片。
- 正向分页对 HeadingBlock 做有界下一块预检：整标题加后文两行能放入一整页时，避免标题孤立在当前页底。极端大标题/大字号仍允许拆分，保留进度与分页能力；不扫描全章。
- CSS 扉页的浮动分栏、旋转、装饰仍属于第三项，本项不宣称还原。

离线验证：原阅读器 69 项通过，新增有界标题换页与大图双向独页回归后 paged_viewport 7 项通过。Android 模拟器运行验收后由用户明确取消，不记录运行通过。

iPhone 用户真机验收（2026-09-08）：用户手动安装后明确确认书名/卷名页眉、本章进度、大图显示及前后翻页均正常。证据为用户反馈，并非代理操作或全量 iOS 回归。静态分析无问题。

第二项状态：DONE（本项约定范围）。2026-09-08 用户取消代理 Android 模拟器验收，按已通过的离线回归、静态分析及用户 iPhone 真机反馈结项；Android 运行未验收。第三项仍为 TODO，未自动启动。


### 本地 EPUB 连续切章（2026-09-08，用户优先修复）

切章不再先销毁当前页。先保存当前进度，再创建待切换会话；待切换页面在当前页后方完成偏好读取和首帧排版，WebView 页等待加载完成。首帧就绪后保留新页面实例并一次替换，期间当前页面保持可见、暂停交互。失败留在原页并提供重试；页面退出会取消待切换会话。最多同时挂载当前页和一个待切换页，没有提前实例化整本书。

待切换会话暂不打开进度写入会话，成功替换后才激活，避免准备或失败时污染当前阅读记录。EPUB 已按书缓存派生内容；另增加最近一次 LocalBookRecord 读取缓存，数据库清单 hash、文件修改时间与长度未变时复用，避免每章重复解码整份 manifest。文件变化重新校验，删除/关闭清缓存。

验证：新增延迟两秒切章仍保留原页、失败重试与分页/滚动交接测试。相关阅读器/本地存储/领域回归 138 项通过；新增缓存复用及篡改失效断言后，managed_local_books 12 项复验通过。静态分析无问题。本轮按用户要求未启动模拟器，实际手机过渡效果仍待用户观察，不宣称零耗时切章。

### 第三项当前实现补记

短版式页通过官方 webview_flutter 4.13.1 离线渲染；按实际应用的浮动/旋转等布局规则识别，不按书名或类名硬编码，正文超过 2000 字符仍走原生阅读器。图片/字体内嵌为 data 资源，移除主动内容，CSP 与禁用 JavaScript 限制外部访问。用户 EPUB 确认提取 1 个版式页；浏览器输出确认分栏、彩色文字、装饰条与作者存在。用户后续明确反馈 iPhone 上可以显示；这不等于全部 CSS 或所有设备验收。

原生正文补充 CSS text-align 与 em text-indent 子集，章节标题也保留显式对齐。旧 EPUB 从保留原件重新生成派生内容，不覆盖原件/清单/进度；章节身份不变，段落样式导致块身份变化时使用现有语义恢复降级。大于单页的特殊内容可纵向滚动，目前按特殊页作为一个阅读单元恢复，不承诺页内精确位置。iOS unsigned Release 编译通过；没有把浏览器截图或编译结果算成模拟器运行。

### 在线正文默认首行缩进（2026-09-08）

在线章节中含中日韩统一汉字的普通正文，在没有来源缩进信息或段首空白时，默认首行缩进两字。已有缩进不叠加，标题、空段、居中 / 靠右段落与图片说明不自动缩进，跨页及长段内部切片的续文不再次缩进。本地 TXT / EPUB 保留原来的来源排版策略。

规则仅用于展示，分页测量、分页绘制、滚动绘制与定位共用，不修改正文、内容身份或持久化字符偏移；已缓存在线章节直接生效。离线阅读器回归 19 项及静态分析通过；本轮未进行模拟器或真机验证。

### 章节标题排版对齐图书观感（2026-09-08）

用户参考图中的章标题采用更强的字号 / 字重、居中和上下留白。本轮原生阅读器将 h1 / h2 及已识别的章节标题段落设为正文字号 1.4 倍、w700、行高 1.3，默认居中；来源显式居中或靠右仍保留。共享间距为段间距 + 64，按上下平分（默认各 42 个逻辑像素）。h3–h6 使用 1.15 倍、w600、段间距 + 28，不强制居中。保持用户选择的字体，不凭截图声称复现苹果字体或原始 CSS；EPUB 独立版式页继续使用书内样式。

测量、绘制、定位共用样式，长标题自然换行，保留既有标题与后文有界同页规则。调整小尺寸分页 fixture 以容纳加大的标题及后文，20 项阅读器离线回归和静态分析通过；未运行模拟器或真机，本轮视觉效果待用户设备观察。

### EPUB 章首 h4 标题补正（2026-09-08）

用户实书 Section001.xhtml 使用 h4 而非 h1/h2，导致上一轮只得到较低级标题的字号和靠左排版。解析器现在将文档首块、且无更高层级标题的 h3–h6 归为章标题（领域 level 2），内部同级标题仍保留原层级；章节名提取包含全部 h1–h6。原始 EPUB 不修改，已导入书籍通过现有原件派生重解析更新；标题块身份可能变化，沿用语义进度恢复降级。24 项离线回归及静态分析通过；未进行手机运行验证。

### 阅读设置重置入口简化（2026-09-08）

按用户要求移除单独的“恢复默认排版”，只保留“恢复默认”，统一恢复全部阅读设置；保留既有操作提示已读状态。同步清理中英文多余文案。
