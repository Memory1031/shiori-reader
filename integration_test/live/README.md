...
只读既定样本，串行至少1秒间隔，无重试，失败即停。内容与图片只在内存数据库/媒体租约中保留；仅首图用于页面挂载，其他图在探针层本地占位，挂载阶段禁止网络。生产代码没有该限制。验收见 [验收摘要](../../docs/validation/README.md) 与 `docs/validation/reading-flow.json`。
