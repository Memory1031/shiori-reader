...
| 本地导入 | TXT / 无 DRM EPUB 导入后自动上架，可离线阅读、目录锚点跳转、恢复进度及管理本地文件 |
...
- [ ] 完善阅读排版与设置。
- [x] 实现本地书架、阅读历史和进度恢复界面。
- [ ] 实现有容量限制的缓存与离线续读。
- [x] 实现本地 TXT / 无 DRM EPUB 导入、上架及离线阅读闭环；平台验收边界见 [LOCAL-005](docs/validation/local-005.md)。
