...
import 'contract_fakes.dart';

/// In-memory protocol example, NOT proof of durable DB-002 transactions/restart.
final class ContractLibrary implements LibraryRepository {
  ...
  Future<Result<BookshelfEntry>> putBookshelf(
    ...
    if (cancellation.isCancelled) return cancelled(Operation.libraryWrite);
  ...
  Future<Result<ReadingProgress?>> getProgress(
    ...
    required CancellationToken cancellation,
      ...
      ? cancelled(Operation.progressRead)
      : Success(_progress[key]);
  @override
  Future<Result<int>> beginProgressSession(
    NovelKey key, {
    required CancellationToken cancellation,
  }) async {
    if (cancellation.isCancelled) return cancelled(Operation.progressWrite);
